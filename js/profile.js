// =======================================================================
// profile.js – Zvekisha Dota Hub (2025 FINAL PREMIUM EDITION)
// HERO STATS + RECENT MATCHES + ITEMS + TOOLTIP + CACHING + THREADS + COMMENTS
// =======================================================================
...
// =======================================================================
// FULL ITEM MAP (short version: only name is important)
// =======================================================================
const itemNames = new Proxy({}, {
  get: (_, id) => _[id] || (
    {
      0:"empty"
    }[id] || "empty"
...
function cacheKey(pid) {
  return "zvek_cache_dota_" + pid;
}
...
function saveCache(pid, data) {
  localStorage.setItem(cacheKey(pid), JSON.stringify({
    timestamp: Date.now(),
    data
  }));
}
...
async function loadDotaData(uid) {
  ...
  const topEl = document.getElementById("topHeroes");
  ...
  if (!pid) return container.innerHTML = "Нема Dota ID.";

  // Convert SteamID64 → Dota32
  ...
  if (cached) {
    return renderDotaProfile(cached, container, topEl, recentEl);
  }
  ...
  renderDotaProfile(data, container, topEl, recentEl);
}
...
function renderDotaProfile(data, container, topHeroesEl, recentEl) {
  ...
  if (!player || !player.profile) {
    container.innerHTML = "Dota профилот е приватен.";
    return;
  }
    ...
    return `
      <div class="match ${win ? "win" : "loss"}">
        ...
        <div class="items-wrapper">
          ...
          <div class="neutral-slot">${renderItem(neutral)}</div>
        </div>
...
async function loadUserThreads(uid) {
  const el = document.getElementById("userThreads");
  ...
  if (snap.empty) {
    el.innerHTML = "Нема теми.";
    return;
  }

  el.innerHTML = snap.docs.map(doc => {
    const t = doc.data();
    return `<div><a href="thread.html?id=${doc.id}" class="tlink">${escapeHtml(t.title)}</a></div>`;
  }).join("");
}
